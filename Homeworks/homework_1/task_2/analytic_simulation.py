...
# Упрощенная версия задачи 2:
# принято k1 = k2 = k3 = k и введен параметр w, где w^2 = k/m.
...
def main():
    ...
    # k1 = k2 = k3 = k, and w^2 = k/m.
    # Original:
    # m*x'' = -k2*x - sin(a)^2*k3*x - cos(a)sin(a)*k3*y
    # m*y'' = -k1*y - cos(a)sin(a)*k3*x - cos(a)^2*k3*y
    # With k1=k2=k3=k and k/m = w^2:
    # x'' = -(1 + sin(a)^2) * w^2 * x - cos(a)sin(a) * w^2 * y
    # y'' = -cos(a)sin(a) * w^2 * x - (1 + cos(a)^2) * w^2 * y
    ode_x = sp.Eq(
        sp.diff(x, t, 2),
        -(1 + sa**2) * w**2 * x - ca * sa * w**2 * y,
    )
    ode_y = sp.Eq(
        sp.diff(y, t, 2),
    ...
    # U = 0.5*m*w^2*(1+sin^2(a))*x^2 + m*w^2*cos(a)sin(a)*x*y
    #     + 0.5*m*w^2*(1+cos^2(a))*y^2
    U = (
        0.5 * m_val * w_val**2 * (1 + sa_val**2) * xs**2
