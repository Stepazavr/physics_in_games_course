...
function _sceneStack() {
  ...
  const wallH = 3;
  const wall1 = makeFigure({ x: [ 6.0, 0.2 + wallH, 0], halfExtents: [0.2, wallH, 6], static: true });
  wall1.invisible = true;
  simulation.bodies.push(wall1);
  const wall2 = makeFigure({ x: [-6.0, 0.2 + wallH, 0], halfExtents: [0.2, wallH, 6], static: true });
  wall2.invisible = true;
  simulation.bodies.push(wall2);
  const wall3 = makeFigure({ x: [0, 0.2 + wallH,  6.0], halfExtents: [6, wallH, 0.2], static: true });
  wall3.invisible = true;
  simulation.bodies.push(wall3);
  const wall4 = makeFigure({ x: [0, 0.2 + wallH, -6.0], halfExtents: [6, wallH, 0.2], static: true });
  wall4.invisible = true;
  simulation.bodies.push(wall4);
...
function _scenePile() {
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    b.q = quaternionFromAxisAngle(normalizeVector([Math.random()-0.5, Math.random()-0.5, Math.random()-0.5]),
                            Math.random() * Math.PI);
    simulation.bodies.push(b);
  }
}
