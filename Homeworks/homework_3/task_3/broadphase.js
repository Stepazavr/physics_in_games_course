function findPotentialPairsBrute(bodies) {
  const pairs = [];
...
function computeGridCellHash(i, j, k) {
  return (((i * 73856093) ^ (j * 19349663) ^ (k * 83492791)) >>> 0);
}

function findPotentialPairsSpatialGrid(bodies, cellSize) {
  const cs = cellSize;
  const inv = 1 / cs;
  const table = new Map();
  ...
  for (let bi = 0; bi < bodies.length; bi++) {
    ...
    for (let k = k0; k <= k1; k++)
      for (let j = j0; j <= j1; j++)
        for (let i = i0; i <= i1; i++) {
          const key = computeGridCellHash(i, j, k);
